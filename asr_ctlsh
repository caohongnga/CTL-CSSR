...
nbpe=4004  # The number of BPE vocabulary.
bpemode=unigram      # Mode of BPE (unigram or bpe).
oov="<unk>"         # Out of vocabulary symbol.
blank="<blank>"     # CTC blank symbol
sos_eos="<sos/eos>" # sos and eos symbole
bpe_input_sentence_size=100000000 # Size of input sentence for BPE.
bpe_nlsyms="<v-noise>"         # non-linguistic symbols list, separated by a comma, for BPE
bpe_char_cover=1.0  # character coverage when modeling BPE

# Language model related
use_lm=false       # Use language model for ASR decoding.
lm_tag=cs_4004_len3s_final #cs_4005_len3s #cs_4000_len3s #cs_4004_len3s   #rnn_lm           # Suffix to the result dir for language model training.
lm_exp=           # Specify the direcotry path for LM experiment.
                  # If this option is specified, lm_tag is ignored.
lm_stats_dir=     # Specify the direcotry path for LM statistics.
lm_config=./conf/train_lm_transformer.yaml  #./conf/train_lm.yaml      # Config for language model training.
lm_args="--max_epoch 60" #best 100         # Arguments for language model training, e.g., "--max_epoch 10".
                  # Note that it will overwrite args in lm config.
use_word_lm=false # Whether to use word language model.
num_splits_lm=1   # Number of splitting for lm corpus.
# shellcheck disable=SC2034
word_vocab_size=23159 # Size of word vocabulary.

# ASR model related
asr_tag=baseline_4004 #cs_pinyin_mono_base_4004_cycle3_5000x2_5503_cycle3 #cs_pinyin_mono_base_4004_cycle3_5000x2_5503x2_cycle4  #cs_pinyin_mono_base_4004_cycle4 #pinyin_mono5503_base_4000_round4 #cs_pinyin_mono6000_base_4000_round4 #cs_pinyin_mono_base_4000_cycle4 #cs_pinyin_mono4800_base_4000_round4 #cs_pinyin_mono_base_4005_round4 #cs_pinyin_mono_base_4005_round4 #cs_pinyin_mono_base_4005_round3 #cs01_pinyin_mono_base_4000_cycle3_tuning #cs_pinyin_mono_base_4005_round2 #cs_pinyin_mono_base_4005 #asr_cs_base_4005_aishell_ted2_01 #asr_baseline_4000 
# cs01_pinyin_mono_base_4000_cycle3_tuning #cs_pinyin_4004_prp #cs_pinyin_mono_base_4004_round3_tuning #cs_pinyin_mono_base_4004_cycle3_tuning #cs_pinyin_mono_base_3500_cycle1 #cs_pinyin_mono_base_4500_cycle1 #cs_pinyin_4500_prp  ##cs_pinyin_mono_base_4004_cycle1     # Suffix to the result dir for asr model training.
...
asr_config=./conf/train_asr_transformer.yaml  # Config for asr model training.
asr_args="--max_epoch 80"     # Arguments for asr model training, e.g., "--max_epoch 10".
...
inference_args="--beam_size 8 --lm_weight 0"   # Arguments for decoding, e.g., "--lm_weight 0.1".
                  # Note that it will overwrite args in inference config.
inference_lm=valid.loss.ave.pth        # Language modle path for decoding.
inference_asr_model=valid.acc.ave.pth #valid.acc.ave_6best.pth #valid.acc.best.pth # ASR model path for decoding. 
                                      # e.g.
...
train_set="train_set5"       # Name of training set.
valid_set="val_set5"       # Name of validation set used for monitoring/tuning network training.
test_sets="dev_man dev_sge" #"val_set5"  #"dev_man_p1 dev_sge_p1 dev_man dev_sge" #"dev_man dev_sge" #"dev_man_p1 dev_sge_p1 dev_man dev_sge" # dev_man_p1 dev_sge_p1"       # Names of test sets. Multiple items (e.g., both dev and eval sets) can be specified.
bpe_train_text=  # Text file path of bpe training set.
lm_train_text=dump/raw/lm_train.txt #data/train/text   # Text file path of language model training set.
lm_dev_text=data/val_set5/text     # Text file path of language model development set.
lm_test_text=data/dev/text    # Text file path of language model evaluation set.
nlsyms_txt=none  # Non-linguistic symbol list if existing.
cleaner=none     # Text cleaner.
g2p=none         # g2p method (needed if token_type=phn).
lang=cs #cscp
 #en      # The language type of corpus.
...
if ! "${skip_eval}"; then
    if [ ${stage} -le 11 ] && [ ${stop_stage} -ge 11 ]; then
        ...
        for dset in ${test_sets}; do
            ...
            LOGFILE="${_dir}/start_time.log"
            TIMESTAMP=$(date)
            echo "$TIMESTAMP start decoding" >> $LOGFILE
            # shellcheck disable=SC2086
            ...
            LOGFILE="${_dir}/stop_time.log"
            TIMESTAMP=$(date)
            echo "$TIMESTAMP stop decoding" >> $LOGFILE
        done
    fi
